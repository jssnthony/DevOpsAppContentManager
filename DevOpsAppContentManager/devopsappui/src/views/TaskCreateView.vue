<template>
  <section class="create">
    <header class="create-header">
      <div>
        <h1 class="title-main">Nueva tarea</h1>
        <p>Agrega una tarea al proyecto</p>
      </div>
    ...
    <form class="card" @submit.prevent="submit">
      <div class="grid">
        ...
        <label class="field">
          <span class="label">Proyecto ID <span class="req">*</span></span>
          <input :value="projectId"
                 class="input"
                 type="text"
                 disabled />
          <span v-if="touched && !projectId" class="hint error">
...
import { insertTask, type ProjectTaskDtoToInsert } from "@/services/projectsTasks.services";
...
const form = reactive<ProjectTaskDtoToInsert>({
  title: "",
  description: "",
});
...
const canSubmit = computed(() => {
  ...
  const projectOk = projectId.value.trim().length > 0;
  return titleOk && projectOk && !saving.value;
});
...
async function submit() {
  ...
  try {
    ...
    error.value = e?.message ?? "Error insertando la tarea";
  } finally {
    saving.value = false;
  }
}
